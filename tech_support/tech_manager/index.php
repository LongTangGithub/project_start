...
if ($action == 'list_technicians') {
    // Get technician data display in table
    ...
    if (empty($first_name) || empty($last_name) || empty($phone) || 
        $email === FALSE || empty($password)) {
            $error = "Invalid technician data. Check all fields and try again.";
            include('../errors/error.php');

    }else{
        // Execute the add_technician method and display it the table
        add_technician($technician_id, $first_name, $last_name, $email, $phone, $password);
        header("Location: .");
